    ...
    <div class="px-4 -mt-24">
        ...
        <div class="card p-6 mb-6">
            <div class="flex flex-col items-center text-center mb-6">
                ...
                <h2 class="text-2xl font-bold text-secondary-900">{{ auth()->user()->name }}</h2>
                <p class="text-secondary-600 mt-1">{{ auth()->user()->email }}</p>
                @if(auth()->user()->department)
                    <span class="badge bg-primary-100 text-primary-600 mt-2">{{ auth()->user()->department }}</span>
                @endif
            </div>
            ...
            <div class="space-y-3 pt-6 border-t border-secondary-200">
                <div class="flex items-center justify-between py-3">
                    ...
                    <span class="font-medium text-secondary-900">{{ ucfirst(auth()->user()->role) }}</span>
                </div>
                <div class="flex items-center justify-between py-3 border-t border-secondary-100">
                    ...
                    <span
                        class="font-medium text-secondary-900">{{ auth()->user()->created_at->format('d M Y') }}</span>
                </div>
        ...
        <div class="card p-6 mb-6">
            ...
            <div class="grid grid-cols-3 gap-4">
                ...
                <div class="text-center">
                    <p class="text-3xl font-bold text-primary-600">{{ $totalCheckIns }}</p>
                    <p class="text-xs text-secondary-600 mt-1">Total Absen</p>
                </div>
                <div class="text-center">
                    <p class="text-3xl font-bold text-success-600">{{ $onTimeCount }}</p>
                    <p class="text-xs text-secondary-600 mt-1">Tepat Waktu</p>
                </div>
                <div class="text-center">
                    <p class="text-3xl font-bold text-danger-600">{{ $lateCount }}</p>
