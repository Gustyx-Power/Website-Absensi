        ...
        <div class="max-w-md w-full">
            ...
            <div class="bg-white rounded-2xl shadow-2xl p-8">
                ...
                @if (session()->has('success'))
                    <div class="mb-4 p-4 bg-success-50 border border-success-200 rounded-lg">
                        <p class="text-success-600 font-medium text-sm">{{ session('success') }}</p>
                    </div>
                @endif

                @if (session()->has('error'))
                    <div class="mb-4 p-4 bg-danger-50 border border-danger-200 rounded-lg">
                        <p class="text-danger-600 font-medium text-sm">{{ session('error') }}</p>
                    </div>
                @endif
            ...
            <div class="text-center mt-8">
                <p class="text-primary-100 text-sm">
                    © {{ date('Y') }} Wengset Technology. All rights reserved.
                </p>
            </div>
        </div>
    </div>
